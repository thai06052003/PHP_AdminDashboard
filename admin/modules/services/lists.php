...
<section class="content">
    <div class="container-fluid">
        <?php if ($checkRoleAdd): ?>
        <a href="<?php echo getLinkAdmin('services', 'add') ?>" class="btn btn-success btn-sm mb-3">
        ...
        <form action="" method="get">
            ...
            <input type="hidden" name="module" value="services"> <!-- ngăn chuyển về trang chủ -->
        </form>
        <?php
        getMsg($msg, $msgType);
        ?>
            ...
            <tbody>
                ...
                ?>
                        <tr>
                            ...
                            <td><?php echo (isFontIcon($item['icon'])) ? html_entity_decode($item['icon']) : '<img src="' . $item['icon'] . '" width="80" />'; ?></td>
                            <td>
                                <a href="<?php echo getLinkAdmin('services', 'edit', ['id' => $item['id']]) ?>">
                                    <?php echo $item['name']; ?>
                                </a>
                            </td>
                            <td>
                            ...
                            <td><?php echo getDateFormat($item['create_at'], 'H:i:s d/m/Y'); ?></td>                            
                            <td class="text-center">
                                <a href="<?php echo getLinkModule('services', $item['id']) ?>" target="_blank" class="btn btn-primary btn-sm">
                                    Xem
                                </a>
                            </td>
                            <?php if ($checkRoleDuplicate): ?>
                            <td class="text-center">
                                <a href="<?php echo getLinkAdmin('services', 'duplicate', ['id' => $item['id']]) ?>" class="btn btn-danger btn-sm">Nhân bản</a>
                            </td>
                            <?php
                            endif;
                            if ($checkRoleEdit):
                            ?>
                            <td class="text-center">
                                <a href="<?php echo getLinkAdmin('services', 'edit', ['id' => $item['id']]) ?>" class="btn btn-warning btn-sm">
                            ...
                            ?>
                            <td class="text-center">
                                <a href="<?php echo getLinkAdmin('services', 'delete', ['id' => $item['id']]) ?>" onclick="confirm('Bạn có chắc chắn muốn xóa?')" class="btn btn-danger btn-sm">
