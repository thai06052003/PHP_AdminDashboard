...
$allCategoryNum = getRows("SELECT id FROM `blog_categories` $filter");
...
$listCategory = getRaw("SELECT *, (SELECT count(blog.id) FROM blog WHERE blog.category_id = blog_categories.id) as blog_count FROM `blog_categories` $filter ORDER BY create_at DESC LIMIT $offset, $perPage");
        ...
        <div class="row">
            ...
            <div class="col-6">
                ...
                <form action="" method="get">
                    ...
                    <input type="hidden" name="module" value="blog_categories"> <!-- ngăn chuyển về trang chủ -->
                </form>
                <hr>
                    ...
                    <tbody>
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="<?php echo getLinkAdmin('blog_categories', '', ['id' => $item['id'], 'view' => 'edit']) ?>">
                                            <?php echo $item['name']; ?>
                                        </a>
                                        <?php echo ' (' . $item['blog_count'] . ')'; ?>
                                    </td>
                                    <td>
                                        <?php echo !empty($item['create_at']) ? getDateFormat($item['create_at'], 'H:i:s d/m/Y') : ''; ?>
                                    </td>
                                    <td class="text-center">
                                        <a href="<?php echo getLinkAdmin('blog_categories', 'duplicate', ['id' => $item['id']]) ?>" class="btn btn-danger btn-sm mb-1">Nhân bản</a>
                                        <a href="<?php echo getLinkModule('blog_categories', $item['id']) ?>" class="btn btn-success btn-sm" target="_blank">Xem</a>
                                    </td>
                                    <td>
                                        <a href="<?php echo getLinkAdmin('blog_categories', '', ['id' => $item['id'], 'view' => 'edit']) ?>" class="btn btn-warning btn-sm">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                    </td>
                                    <td>
                                        <a href="<?php echo getLinkAdmin('blog_categories', 'delete', ['id' => $item['id']]) ?>" onclick="confirm('Bạn có chắc chắn muốn xóa?')" class="btn btn-danger btn-sm">
